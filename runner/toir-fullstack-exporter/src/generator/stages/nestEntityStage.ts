...
function buildNestUserPrompt(slice: EntitySlice, kebab: string, allowedPrefix: string): string {
  return [
    ...
    '',
    'Import these by path; DO NOT redefine them. The depth of `../` segments depends',
    'on the importing file location:',
    '',
    ...
    "  `import { PaginatedResponse, ListQueryParams } from '../../../shared/pagination';`",
    '',
    'Filename convention for enums: kebab-case derived from PascalCase, no pluralization',
    '(e.g. `EquipmentStatus` → `equipment-status.enum.ts`, `Priority` → `priority.enum.ts`).',
    '',
    ...
    'See `backend-rules.md` (system prompt) for the full DTO/service typing rules.',
    '',
